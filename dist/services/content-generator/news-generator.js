...
class NewsGenerator {
    constructor(options = {}) {
        this.basePrompt = "Você é um jornalista profissional. Reescreva a seguinte notícias em um formato jornalístico profissional, adicionando um título atrativo, subtítulo e organizando o texto em parágrafos coesos. Mantenha todas as informações importantes mas melhore a estrutura e a apresentação:";
        const apiKey = options.apiKey || process.env.OPENAI_API_KEY || "";
        if (!apiKey) {
            throw new errors_1.ApiKeyError();
        }
     ...
     */
    async generateNews(newsList) {
        if (!newsList || newsList.length === 0) {
            throw new errors_1.InvalidInputError("Lista de notícias vazia ou inválida");
        }
        try {
            ...
            for (let i = 0; i < batches.length; i++) {
                ...
                try {
                    ...
                    logger_1.loggerError.error(`Erro ao processar lote ${i + 1}`, {
                        batchNumber: i + 1,
     ...
     */
    createPrompt(newsList) {
        let prompt = `\nVocê deve criar ${this.newsAmount} notícias com no máximo ${this.maxCharacters} caracteres cada uma.
        ...
        "news": [
          {
            ...
            return `Notícia ${index + 1}:
                ...
                - Imagem: ${news.imageUrl && news.imageUrl.length > 0
                ? news.imageUrl[0]
                : "Não possui"}
            ...
            .join("\n\n")}
                ...
                4. Você deve retornar exatamente ${this.newsAmount} notícias no array "news"
5. Dentro do conteúdo da notícia não adicione imagens, nem links, apenas o texto formatado como um HTML
 `;
     ...
     */
    async callApiWithPrompt(prompt) {
        try {
            ...
            if (!response || !response.choices || response.choices.length === 0) {
                throw new errors_1.ContentGenerationError("Resposta da API vazia ou inválida");
            }
    ...
    processApiResponse(responseContent) {
        try {
            ...
            throw new errors_1.ContentGenerationError(`A resposta não é um JSON válido: ${parseError.message}`);
        }
    }
    /**
    ...
    handleApiError(error) {
        if (error instanceof openai_1.default.APIError) {
            ...
            if (status === 401) {
                throw new errors_1.ApiKeyError(`Erro de autenticação: ${message}`);
            }
            else if (status === 429) {
                throw new errors_1.ApiLimitError(`Limite de requisições excedido: ${message}`);
            }
            else if (status === 404 || status === 503) {
                throw new errors_1.ModelUnavailableError(`Modelo não disponível: ${message}`);
            }
            throw new errors_1.ContentGenerationError(`Erro na API do OpenAI: ${message}`);
        }
        throw new errors_1.ContentGenerationError(`Erro ao gerar notícia: ${error.message}`);
    }
    /**
     * Define o número de notícias a serem geradas
     * @param newsAmount Número de notícias
     */
    setNewsAmount(newsAmount) {
